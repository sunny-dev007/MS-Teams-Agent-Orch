...
async def summarize_documents(state: AgentState) -> AgentState:
    if not settings.enable_doc_knowledge:
        return {
            **state,
            ...
            "notification_text": doc_knowledge_offline(agent_label="Doc Insights Agent"),
        }
    ...
    if _doc_upload.message_expects_teams_attachment(raw) and not _doc_upload.extract_share_links(raw):
        return {
            **state,
            "status": "failed",
            "handled_by": AGENT_NAME,
